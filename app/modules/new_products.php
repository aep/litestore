...
if ((!isset ($new_products_category_id)) || ($new_products_category_id == '0')) 
{
    if (GROUP_CHECK == 'true')
        ...
        $new_products_query = "SELECT * FROM
                            ...
                            ".$group_check."
                            ".$fsk_lock."
                            and p.products_status = '1' and pd.languages_id = '".(int) $_SESSION['languages_id']."'
...
if (sizeof($module_content) >= 1) {
    ...
    $module_smarty->assign('CATEGORIES_NAME', "");	
	...
	// set cache ID
	 if (!CacheCheck()) {
		...
		if ((!isset ($new_products_category_id)) || ($new_products_category_id == '0')) {
			$module = $module_smarty->fetch('module/new_products_default.html', $cache_id);
		} else {
			$module = $module_smarty->fetch('module/new_products.html', $cache_id);
		}
	}

...
function module()
{
    ...
    // New VAT CHECK END
        ...
        {
            global $db;
		    $check_email_query = $db->prepare("select count(*) as total from ".TABLE_CUSTOMERS." where customers_email_address = ? and account_type = '0'");
            $check_email_query->execute($email_address);
        ...
        $campaign_check_query_raw = "SELECT *
		    ...
		    if (ACTIVATE_GIFT_SYSTEM == 'true') {
			    ...
			    if (NEW_SIGNUP_DISCOUNT_COUPON != '') {
				    ...
				    $coupon_desc_query = xtc_db_query("select * from ".TABLE_COUPONS_DESCRIPTION." where coupon_id = '".$coupon_id."' and languages_id = '".(int) $_SESSION['languages_id']."'");
    ...
    if (ACCOUNT_GENDER == 'true') {
	    ...
	    $smarty->assign('INPUT_FEMALE', xtc_draw_radio_field(array ('name' => 'gender', 'suffix' => FEMALE ), 'f'));
	    $smarty->assign('INPUT_MALE', xtc_draw_radio_field(array ('name' => 'gender', 'suffix' => MALE, 'text' => (xtc_not_null(ENTRY_GENDER_TEXT) ? '<span class="inputRequirement">'.ENTRY_GENDER_TEXT.'</span>' : '')), 'm'));
    ...
    $smarty->assign('language', $_SESSION['language']);
    $smarty->caching = 0;
    return $smarty->fetch('module/create_account.html');
}
?>

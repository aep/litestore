...
/* -----------------------------------------------------------------------------------------
   ...
   Copyright (cxtc_format_price) 2003 ReStore
   -----------------------------------------------------------------------------------------
   based on: 
...
function module()
{
    ...
    if (isset ($_POST['action']) && ($_POST['action'] == 'process')) {
	    ...
	    if ((xtc_count_shipping_modules() > 0) || ($free_shipping == true)) {
		    if ((isset ($_POST['shipping'])) && (strpos($_POST['shipping'], '_'))) {
			    ...
			    if (true || ($_SESSION['shipping'] == 'free_free')) {
    ...
    $breadcrumb->add(NAVBAR_TITLE_1_CHECKOUT_SHIPPING, '/checkout');
    $breadcrumb->add(NAVBAR_TITLE_2_CHECKOUT_SHIPPING, '/checkout/shipping' );
    
    $smarty->assign('FORM_ACTION', xtc_draw_form('checkout_address',FILENAME_CHECKOUT_SHIPPING).xtc_draw_hidden_field('action', 'process'));
    $smarty->assign('ADDRESS_LABEL', xtc_address_label($_SESSION['customer_id'], $_SESSION['sendto'], true, ' ', '<br />'));
    
    $smarty->assign('BUTON_CONTINUE',  IMAGE_BUTTON_CONTINUE);
    ...
    if (xtc_count_shipping_modules() > 0) 
    {
        ...
        {
            $smarty->assign('FREE_SHIPPING_TITLE', FREE_SHIPPING_TITLE);
        ...
        else 
        {
            ...
            $smarty->assign('shipping_data', $quotes);
        }
    }
    ...
    $smarty->caching = 0;
    return $smarty->fetch(CURRENT_TEMPLATE.'/module/checkout_shipping.html');
}
?>
...
{
    $smarty=new Smarty;
    global $breadcrumb,$main,$messageStack;
    ...
    while ($addresses = xtc_db_fetch_array($addresses_query)) {
	    ...
	    $addresses_data[] = array (
            ...
            'URL_EDIT' => xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'edit='.$addresses['address_book_id']), 'URL_DELETE' => xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'delete='.$addresses['address_book_id']), 'ADDRESS' => xtc_address_format($format_id, $addresses, true, ' ', '<br />'), 'PRIMARY' => $primary);
    
    }
    $smarty->assign('addresses_data', $addresses_data);
    
    $smarty->assign('BUTTON_BACK', '<a href="'.xtc_href_link(FILENAME_ACCOUNT, '', 'SSL').'">'. IMAGE_BUTTON_BACK.'</a>');
    
    if (xtc_count_customer_address_book_entries() < MAX_ADDRESS_BOOK_ENTRIES) {
    
	    $smarty->assign('BUTTON_NEW', '<a href="'.xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS, '', 'SSL').'">'. IMAGE_BUTTON_ADD_ADDRESS.'</a>');
    }
    ...
    $smarty->caching = 0;
    return $smarty->fetch(CURRENT_TEMPLATE.'/module/address_book.html');
}
?>
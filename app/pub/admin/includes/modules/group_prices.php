<?php
    ...
    require_once (DIR_FS_INC.'../classes/price.php');
...
<?php
            ...
            $p=new Price($pInfo->products_id);
            $p->setCustomerStatus($group_data[$col]['STATUS_ID']);

            if (PRICE_IS_BRUTTO == 'true') 
            {
                $products_price = xtc_round($p->fetch() * ((100 + xtc_get_tax_rate($pInfo->products_tax_class_id)) / 100), PRICE_PRECISION);
            } 
            else 
            {
                $products_price = xtc_round($p->fetch(), PRICE_PRECISION);
            }
?>
...
<?php
            ...
            $staffel_query = xtc_db_query("SELECT
                products_id,
                quantity,
                price as personal_offer
                FROM
                prices
                WHERE
                products_id = '".$pInfo->products_id."' AND quantity != 1
                AND
                customers_status_id='".$group_data[$col]['STATUS_ID']."'

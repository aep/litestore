...
var dateScheduled = new ctlSpiffyCalendarBox("dateScheduled", "new_banner", "date_scheduled","btnDate2","<?php echo $bInfo->date_scheduled; ?>",scBTNMODE_CUSTOMBLUE);
    ...
    <tr><?php echo xtc_draw_form('new_banner', FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo xtc_draw_hidden_field('banners_id', $bID); ?>
                    <input name="banners_group"  value="banner" type="hidden" /> 
        ...
        <tr>
            ...
            <td class="main">
                ...
                <?php
                    $bannerdir= realpath(DIR_FS_CATALOG.'../images/banner/');
                    if ($dir = opendir($bannerdir)) {
                        while (($banners = readdir($dir)) !== false) {
                            if (is_file($bannerdir.'/'.$banners) and ($banners != "CVS") and ($banners != ".") and ($banners != "..")) {
                                $back_array[] = array ('id' => $banners, 'text' => $banners);
                            }
                        }
                        closedir($dir);
                        sort($back_array);
                        echo $bannerdir."/&nbsp;";
                        echo xtc_draw_pull_down_menu("banners_image_local", $back_array, $bInfo->banners_image);
                    }
                ?>

...
function module()
{
    ...
    if (isset ($_POST['action']) && (($_POST['action'] == 'process') || ($_POST['action'] == 'update'))) {
        ...
        global $gender,$company,$firstname,$lastname,$street_address,$suburb,$postcode,$city,$country,$zone_id,$state;
    ...
    if (isset ($_GET['delete']) == false)
	    $action = xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS, (isset ($_GET['edit']) ? 'edit='.$_GET['edit'] : ''), 'SSL');
    ...
    if (isset ($_GET['delete'])) {
	    $smarty->assign('delete', $_GET['delete']);
	    $smarty->assign('ADDRESS', xtc_address_label($_SESSION['customer_id'], $_GET['delete'], true, ' ', '<br />'));
	    $smarty->assign('BUTTON_BACK', '<a href="'.xtc_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL').'">back</a>');
	    $smarty->assign('BUTTON_DELETE', '<a href="'.xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'delete='.$_GET['delete'].'&action=deleteconfirm', 'SSL').'">'.IMAGE_BUTTON_DELETE.'</a>');
    } else {
	    include (DIR_WS_MODULES.'address_book_details.php');
    
	    if (isset ($_GET['edit']) && is_numeric($_GET['edit'])) {
		    $smarty->assign('BUTTON_BACK', '<a href="'.xtc_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL').'">Zur&uuml;ck</a>');
            $smarty->assign('update', '1');

		    $smarty->assign('BUTTON_UPDATE', xtc_draw_hidden_field('action', 'update').xtc_draw_hidden_field('edit', $_GET['edit']).'<input type="submit">');
		    ...
		    $smarty->assign('BUTTON_BACK', '<a href="'.$back_link.'">back</a>');
		    $smarty->assign('BUTTON_UPDATE', IMAGE_BUTTON_CONTINUE);
    ...
    $smarty->caching = 0;
    return $smarty->fetch(CURRENT_TEMPLATE.'/module/address_book_process.html');
}
?>
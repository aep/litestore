...
function fmViewError(error) {
  ...
  fmOpenDialog('fmError', error, null, null, x, y);
}
...
function fmOpenDialog(id, text, object, perms, x, y) {
  var f, e, i, start;
  ...
  if(f = document.forms[id]) {
    ...
    if(object) {
      if(f.fmObject) f.fmObject.value = object;
      if(f.fmName) f.fmName.value = object;
    }

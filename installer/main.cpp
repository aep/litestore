...
#define RESDIR   "/root/installer/res/"
#define SVN      "svn://sophia.ibcsolutions.de/svn/restore/production/"
#define TARGET   "/home/"
#define TARGET_B   "/html/restore/"

#define SQLPWD ""
#define VHOSTS   "/etc/httpd/vhosts/"
...
int addShop(QString user)
{
...

    {
        ...
        systemEx ("chmod o+x "  TARGET + installerValues["UNIXUSER"]);
...

    {
        ...
        StdOut<<"\n\n===>production checkout\n";
        systemEx("cd " TARGET "/"  + installerValues["UNIXUSER"] + TARGET_B " ; svn co " SVN + " . " );

        StdOut<<"\n\n===>make templates_c world writeable\n";
        systemEx("chmod o+w " TARGET "/"  + installerValues["UNIXUSER"] + TARGET_B "/app/templates_c ");

        StdOut<<"\n\n===>make templates world writeable\n";
        systemEx("chmod o+w " TARGET "/"  + installerValues["UNIXUSER"] + TARGET_B "/templates ");


        StdOut<<"\n\n===>make images world writeable\n";
        systemEx("chmod o+w " TARGET "/"  + installerValues["UNIXUSER"] + TARGET_B "/images -R ");
    ...
    }
    {
        ...
        StdOut<<"\n\n===>sql insert\n";
            ...
            QTemporaryFile tempsql;
            tempsql.open();
            QTextStream(&tempsql)<<xk;
            tempsql.flush();

            systemEx(("mysql "+installerValues["UNIXUSER"]+" < "+tempsql.fileName()).toLocal8Bit());
